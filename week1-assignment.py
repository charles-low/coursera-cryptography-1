#!/usr/bin/env python
"""
Coursera Cryptography I : Week 1 assignment

when XOR space with alphabets [a-zA-Z], the case will swap.

  20 00100000
A 41 01000001
Z 5A 01011010
a 61 01100001
z 7A 01111010
_ 5F 01011111

Side note: you can change the case of ASCII alphabets via the following:
 'X' | ' ' = 'x'
 'x' & '_' = 'X'

Let c be cipher text and m be message, k be key
Given any two cipher text:
    c1 ^ c2 = (m1 ^ k) ^ (m2 ^k)
            = m1 ^ m2

For character position i, if c1[i] ^ c2[i] is alphbet, then m1[i] or m2[i] could be space and the other one could be alphbet.
Starting from c1, we XOR with c2, c3, c4, c5 as x[i]
if all result lies in 41-5A, 61-7A, then M1[i] should be space.
We can try to recover the key via XOR space and c1[i]

Then we repeat the same process to c2...cn

"""

c1=""
c2="234c02ecbbfbafa3ed18510abd11fa724fcda2018a1a8342cf064bbde548b12b07df44ba7191d9606ef4081ffde5ad46a5069d9f7f543bedb9c861bf29c7e205132eda9382b0bc2c5c4b45f919cf3a9f1cb74151f6d551f4480c82b2cb24cc5b028aa76eb7b4ab24171ab3cdadb8356f"
c3="32510ba9a7b2bba9b8005d43a304b5714cc0bb0c8a34884dd91304b8ad40b62b07df44ba6e9d8a2368e51d04e0e7b207b70b9b8261112bacb6c866a232dfe257527dc29398f5f3251a0d47e503c66e935de81230b59b7afb5f41afa8d661cb"
c4=""
c5="3f561ba9adb4b6ebec54424ba317b564418fac0dd35f8c08d31a1fe9e24fe56808c213f17c81d9607cee021dafe1e001b21ade877a5e68bea88d61b93ac5ee0d562e8e9582f5ef375f0a4ae20ed86e935de81230b59b73fb4302cd95d770c65b40aaa065f2a5e33a5a0bb5dcaba43722130f042f8ec85b7c2070"
c6=""
c7=""
c8="315c4eeaa8b5f8bffd11155ea506b56041c6a00c8a08854dd21a4bbde54ce56801d943ba708b8a3574f40c00fff9e00fa1439fd0654327a3bfc860b92f89ee04132ecb9298f5fd2d5e4b45e40ecc3b9d59e9417df7c95bba410e9aa2ca24c5474da2f276baa3ac325918b2daada43d6712150441c2e04f6565517f317da9d3"
c9=""
c10="466d06ece998b7a2fb1d464fed2ced7641ddaa3cc31c9941cf110abbf409ed39598005b3399ccfafb61d0315fca0a314be138a9f32503bedac8067f03adbf3575c3b8edc9ba7f537530541ab0f9f3cd04ff50d66f1d559ba520e89a2cb2a83"
c11="32510ba9babebbbefd001547a810e67149caee11d945cd7fc81a05e9f85aac650e9052ba6a8cd8257bf14d13e6f0a803b54fde9e77472dbff89d71b57bddef121336cb85ccb8f3315f4b52e301d16e9f52f904"

def is_alphabet(b):
    return ( b >= 0x41 and b < 0x5A ) or (b >= 0x61 and b <= 0x7A ) 

def xor_bytes(bytes1, bytes2):
    return list(map(lambda a, b: a^ b, bytes1, bytes2))

def find_space_character(k_dict, from_bytestring, against_bytestrings, threshold=0):
    for i in range(len(from_bytestring)):
        count = 0
        # get a list of bytes in same position
        bytes = [ b[i] for b in against_bytestrings if len(b) > i ]
        xor_bytes = [ from_bytestring[i] ^ b for b in bytes]

        for byte in xor_bytes:
            if not is_alphabet(byte) and byte != 0x00:
                count += 1
                
        # we assume the character is space, then we recover the key using XOR
        previous_count = k_dict.get(i, (None, None))
        if previous_count[0] is None or previous_count[0] > count:
            k_dict[i] = (count, from_bytestring[i] ^ ord(' '))

def decrypt(k_dict, B):
    for i in range(len(B)):
        print("-----------------------------------------------------------------------------------")
        print("*** Attempt to decrypt C%s ***" % i)
        c = B[i]
        result = []
        pos = []
        opt = []
        for j in range(len(c)):
            pos.append('{:02x}'.format(j))
            if j in k_dict.keys():
                opt.append('{:02x}'.format(k_dict[j][1]))
                result.append(chr(k_dict[j][1] ^ c[j]))
            else:
                opt.append('XX')
                result.append('#')

        print("POSITION:      %s" % ' '.join(pos))
        print("OTP RECOVERED: %s" % ' '.join(opt))
        print("CIPHER:        %s" % ' '.join('{:02x}'.format(k) for k in c))
        print("DECRYPTED:     %s" % '  '.join(result))
        print("OUTPUT STRING: %s" % ''.join(result))

def main():
    k_dict = {}
    C = [c1,c2,c3,c4,c5,c6,c7,c8,c9,c10,c11]
    # convert C into bytes string
    B = [bytes.fromhex(c) for c in C ]
    # build up the OTP by searching for space characters
    for i in range(len(B)):
        T = B.copy()
        b = T.pop(i)
        find_space_character(k_dict,b,T,1)

    # Fixing individual position after run the script and review the output
    k_dict[2] = (0,0x4e ^ 0x20)
    k_dict[7] = (0,0xa9 ^ ord('e'))
    k_dict[10] = (0,0x41 ^ ord('t'))
    k_dict[0x0e] = (0,0xe6 ^ ord('s'))
    k_dict[0x15] = (0,0x1c ^ ord('c'))
    k_dict[0x19] = (0,0x1a ^ ord('e'))
    k_dict[0x22] = (0,0x52 ^ ord('a'))
    
    # decrypt 
    decrypt(k_dict, B)

main()